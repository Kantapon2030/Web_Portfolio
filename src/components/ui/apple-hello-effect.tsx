...
export type AppleHelloEnglishEffectProps = Omit<
  ComponentProps<typeof motion.svg>,
  "onAnimationComplete"
> & {
  /**
   * Speed multiplier for handwriting animation.
   * Default is 1. Higher is slower (durationScale) or faster depending on scale.
   */
  speed?: number;
...
export function AppleHelloEnglishEffect({
  ...
  // If durationScale is provided directly, use it; otherwise use speed factor
  const factor = durationScale ?? (speed > 0 ? 1 / speed : 1);
  const calc = (x: number) => x * factor;

  return (
    <motion.svg
      className={`h-20 sm:h-28 md:h-36 text-white w-auto max-w-full ${className}`}
    ...
    >
      ...
      <motion.path
        ...
        transition={{
          ...
          ease: "easeInOut",
          delay: calc(0.6),
          opacity: { duration: calc(0.6), delay: calc(0.6) },
        }}
...
// Named alias matching AppleHelloEffectEnglish
export const AppleHelloEffectEnglish = AppleHelloEnglishEffect;
